// Script to upload Reese's cake image to Supabase Storage and update database
const { createClient } = require('@supabase/supabase-js');
const fs = require('fs');
require('dotenv').config({ path: '.env.local' });

async function uploadReesesCake() {
  console.log('🍰 Uploading Reese\'s cake image to Supabase Storage...\n');
  ...
  try {
    ...
    const imagePath = 'public/menu-images-compressed/desserts/reeses-cake.webp';
    ...
    const fileName = 'reeses-cake.webp';
    ...
    // Update the database with the Supabase URL for ID 20
    console.log('\n📝 Updating ID 20 in item_images table with Reese\'s cake URL...');

    const { error: updateError } = await supabase
      .from('item_images')
      .update({
        image_url: supabaseUrl,
        alt_text: 'Reeses Cake - Napoli Pizzeria'
      })
      .eq('id', 20);
    ...
    console.log('\n🎉 Reese\'s cake upload and database update completed successfully!');
